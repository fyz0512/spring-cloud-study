...
public class HttpRequestDeviceUtils {
	...
	final static Logger Log = LoggerFactory.getLogger(HttpRequestDeviceUtils.class);
    ...
    private static String[] pcUserAgents=new String[]{
    "Windows 98",
    "Windows ME",
    "Windows 2000",
    "Windows XP",
    ...
    public static boolean isMobileDevice(HttpServletRequest request){
            ...
            for (int i = 0; via!=null && !via.trim().equals("") && i < mobileGateWayHeaders.length; i++) {
                    if(via.contains(mobileGateWayHeaders[i])){
                            mobileFlag = true;
                            clientCharacter = mobileGateWayHeaders[i]+";";
                            break;
                    }
            }
            for (int i = 0;!mobileFlag && userAgent!=null && !userAgent.trim().equals("") && i < mobileUserAgents.length; i++) {
                    if(userAgent.contains(mobileUserAgents[i])){
                            mobileFlag = true;
                            clientCharacter += mobileUserAgents[i]+";";
                            break;
                    }
            }
            for (int i = 0; userAgent!=null && !userAgent.trim().equals("") && i < pcUserAgents.length; i++) {
                    if(userAgent.contains(pcUserAgents[i])){
                    		clientCharacter += pcUserAgents[i]+";";
                            pcFlag = true;
                    }
            }
            Log.debug(clientCharacter);
